...
def analyze_data(directory):
    ...
    for filepath in get_files(directory, ".npy"):
        ...
        try:
            ...
            sns.distplot(data.flatten(), bins=50, kde=False)
            plt.title(f"Histogram of data in {filepath}")
            plt.show()

            # Additional code for integrity check
            if data.size > 0 and data.shape[0] > 0:
                print(f"Data in {filepath} is non-empty and has a non-zero number of rows")
            else:
                print(f"Data in {filepath} is either empty or has zero rows")
